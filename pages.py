...
def PrintSlideshowImage(Address,Content):
	...
	return '<body style = "font-family:' + font +'!important;background-color:black;margin:0px;padding:0px;"><img src="' + Address + 'img/' + image + '" style ="max-height: 100%;max-width: 100%;position: fixed;top: 50%;left: 50%;transform: translate(-50%, -50%);"></src>';
...
def GetPage(Type,Content,Configuration):
	...
	if str(Type) in functions:
		return GetBackbone(functions[str(Type)](Configuration["ADDR"],Content),debug);
	else:
		return GetBackbone(functions['0'](Configuration["ADDR"],Content),debug)
